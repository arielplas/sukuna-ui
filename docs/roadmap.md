...
- 2026-09-21 · Variants wave (owner-approved sequencing from the Component Variant Proposals doc; 8 commits, all additive → 7 minor changesets): (1) size scale standardized to sm|md|lg — lg added to Checkbox/Switch/Progress/Select/RadioGroup; (2) Toast tone info|success|warning|danger mirroring Alert (no default); (3) Card tone premium (premium-dim border + 6% color-mix tint, DECISION(open): border+tint), interactive (hover lift + focus-within ring, affordance only), glow; (4) Tabs variant underline|pill (well track, surface segment lighter in both themes), size, fitted; (5) shared form-control variant filled|outline|ghost on Input/Select/Combobox/NumberField (+ Combobox size); (6) Badge/Chip variant soft|solid|outline via one literal tone×variant map with an internal 'auto' default that preserves the mixed original look, Chip selected (data-selected); (7) Skeleton animation shimmer (reuses sk-shine), Table density|striped|hoverable via descendant selectors. Every default reproduces the prior look exactly. 328 tests, 100% cov on touched files · (variants-wave branch)
- 2026-09-21 · Brand kit integrated (brand/ = source of truth: Ember Gate mark, wordmark, lockups, favicon set, social/OG, Storybook logos, textures; excluded from Biome + npm). Showcase: full favicon set + site.webmanifest + new og.svg/og.png (1200×630) and og:image:alt; Storybook manager brandImage via staticDirs → brand/storybook; README hero banner (raw GitHub URL so npm renders it too). Manual steps left for the owner: GitHub → Settings → Social preview (brand/social/github-social-preview.png) and the npm/GitHub avatar (brand/social/avatar-512.png) · (brand-assets branch)
