...
| 8 | Consumer matrix | [~] | vite-react18, vite-react19, next-app-router, remix all build with zero hydration warnings | vite-react19, vite-react18, **next-app-router all build** via `bun link`. Next `build` prerenders — RSC boundary validated (server page imports static Text/Badge/Card; client Demo island). **Remaining:** remix example + runtime hydration smoke |
